...
export default function CheptelDetails({ vache }: Props) {
    ...
    return (
        <AppLayout title={`${__('Détails du bovin')} ${vache.numero_ticket}`}>
            <Head title={`${__('Bovin')} ${vache.numero_ticket}`} />
                    ...
                    <div className="flex-1">
                        ...
                        {isSold && vache.prix_vente && (
                            <div className="mb-2 text-sm text-slate-600 font-medium bg-red-50 text-red-700 px-3 py-1.5 rounded-lg inline-flex items-center gap-2">
                                {__('Vendu à')} {vache.prix_vente} {__('DH le')} {new Date(vache.date_vente!).toLocaleDateString('fr-FR')}
                            </div>
                        )}
                        <div className="space-y-2 mb-3">{healthBadge(vache.statut_sante)}
                            {vache.date_naissance && <p className="text-slate-500 text-sm"><strong>{__('Née le :')}</strong> {new Date(vache.date_naissance).toLocaleDateString('fr-FR')}{vache.age !== null ? ` (${vache.age} ${__('ans')})` : ''}</p>}
                            <p className="text-slate-500 text-sm"><strong>{__('Origine :')}</strong> {vache.origine === 'ne_sur_ferme' ? __('Née à la ferme') : __('Achetée')}</p>
                            {vache.fichier_documents && (
                                <p className="text-slate-500 text-sm mt-2">
                                    <strong>{__('Fichier :')}</strong>{' '}
                                    <a href={vache.fichier_documents} target="_blank" rel="noreferrer" className="text-brand-600 hover:text-brand-800 underline">{__('Voir le document')}</a>
                                </p>
                            )}
                        </div>
                        <div>
                            <h3 className="text-sm font-semibold text-slate-600 mb-1">{__('Propriétaire(s) :')}</h3>
                            {vache.clients.length > 0 ? <ul className="text-sm text-slate-600 space-y-0.5">{vache.clients.map(c => <li key={c.id}>• {c.user?.nom} {c.user?.prenom} ({(c.pivot.part_possedee * 100).toFixed(0)}%)</li>)}</ul> : <p className="text-slate-400 text-sm">{__('Appartient à la ferme')}</p>}
                        </div>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="card-premium">
                        ...
                        <div className="p-4 bg-slate-50 rounded-xl border border-slate-100">
                            <h3 className="text-sm font-semibold text-slate-600 mb-2">{__('Derniers Événements')}</h3>
                            {vache.health_statuses.length > 0 ? <ul className="text-sm space-y-2">{vache.health_statuses.slice(0, 4).map(h => <li key={h.id} className="flex justify-between border-b border-slate-100 pb-1.5"><span className="text-slate-700">{h.type === 'sickness' ? __('Maladie') : h.type === 'pregnancy' ? __('Gestation') : __('Visite routine')}</span><span className="text-slate-400 text-xs">{h.date_debut}</span></li>)}</ul> : <p className="text-xs text-slate-400">{__('Aucun événement enregistré')}</p>}
                        </div>
                    </div>
                </div>
            ...
            {showHealthModal && createPortal(<div className="modal-overlay"><div className="modal-panel max-w-2xl max-h-[80vh] overflow-y-auto"><div className="flex justify-between items-center p-6 border-b border-slate-100"><h3 className="text-lg font-bold font-display">{__('Archive Santé')}</h3><button onClick={() => setShowHealthModal(false)} className="text-slate-400 hover:text-slate-600 transition-colors"><X className="h-5 w-5" /></button></div><div className="p-6"><table className="table-premium"><thead><tr><th>{__('Statut')}</th><th>{__('Début')}</th><th>{__('Fin')}</th></tr></thead><tbody>{vache.health_statuses.map(h => <tr key={h.id}><td>{h.type === 'sickness' ? __('Maladie') : h.type === 'pregnancy' ? __('Gestation') : __('Visite routine')}</td><td>{h.date_debut}</td><td>{h.date_fin || __('En cours')}</td></tr>)}</tbody></table></div></div></div>, document.body)}
        </AppLayout>
    );
}
